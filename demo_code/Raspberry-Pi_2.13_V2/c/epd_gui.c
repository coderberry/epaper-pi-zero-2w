        ...
        for (col_bit = 0; col_bit < Font->w; col_bit++ )
        {
            if (FONT_BACKGROUND == Color_Background) 
            {
                if (*ptr & (0x80 >> (col_bit % 8)))
                    Gui_SetPixel(x+row, y+col_bit, Color_Foreground); 
            }
            else
            {
                if (*ptr & (0x80 >> (col_bit % 8)))
                    ...
                    Gui_SetPixel(x+row, y+col_bit, Color_Background); 
...
void Gui_Draw_Str(uint16_t x, uint16_t y, const char *str_char, FONT* Font, uint16_t Color_Background, uint16_t Color_Foreground )
{
    ...
    while(*str_char != '\0')
    {
        if((dy+Font->w)>Image.h)//wrap the line if the x direction has reached the boundary
        {
            dx += Font->h;
            dy = y;
        }
        if((dx+Font->h)>Image.w)//wrap the line if the y direction has reached the boundary
        {
            dx = x;
            dy += Font->w;
        }
