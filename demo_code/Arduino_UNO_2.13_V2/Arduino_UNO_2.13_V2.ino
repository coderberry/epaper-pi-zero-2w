...
void EPD_WhiteScreen_ALL(const unsigned char * datas)
{
	unsigned int i,j,x_bytes,index; 
  unsigned char tmp;	
  ...
  for(j=0;j<MAX_COLUMN_BYTES;j++)//
  {
      ...
      index = i+j*x_bytes;
			if(i==0)  
			{
			   Epaper_Write_Data((pgm_read_byte(&datas[index])>>6)|0xfc);
			}
			else
			{
				Epaper_Write_Data((pgm_read_byte(&datas[index-1])<<2)|(pgm_read_byte(&datas[index])>>6));
			}         
		}
	}
